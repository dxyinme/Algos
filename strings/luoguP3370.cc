...
int main()
{
  ...
  std::array<std::tuple<int, int>, 1> mods;
  mods[0] = {300, 998244353};
  ...
  {
    std::cin >> str;
    MultiHash<char, int, 1> h((const char *)str.c_str(), str.size(), process, mods);
    auto res = h.GetSub(0, str.size() - 1);
    tr.insert(res[0]);
  }
